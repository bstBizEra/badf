...
class FamilyNameResolutionTests(unittest.TestCase):
    ...
    def test_declared_future_entries_carry_their_provenance(self):
        ...
        # BADF-REV #259). Resolution is a WORD-BOUNDARY token match, not a substring test:
        # raw `in` admitted fragments that are mere prefixes of a real id (`SEC`, `SEC-I1`)
        # and the empty string -- the same vacuous-containment root as BADF-QA's finding 1.
        # What still passes, deliberately: a REAL token that is wrong provenance (an adjacent
        # id like SEC-I15, a gate id like G08) -- review's to catch, per the DECLARED SCOPE
        # OF THE LIST CONTROLS above.
        ...
        for e in entries:
            ...
            token = re.compile(rf"(?<![\w-]){re.escape(cited)}(?![\w-])")
            hits = [p for p in doctrine if token.search(p.read_text(encoding="utf-8"))]
